...
#+NAME: redirect-test
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var dpdk_data=dpdk_test :var xdp_data=xdp_redirect_data :var linux_data=linux_fwd
dpdk = np.array(dpdk_data)
xdp = np.array(xdp_data)
linux = np.array(linux_data)
plt.plot(dpdk[:,0], dpdk[:,2]/10**6, marker='o', label="DPDK")
plt.plot(xdp[:,0], xdp[:,3]/10**6, marker='s', label="XDP 256")
plt.plot(redir[:,0], redir[:,2]/10**6, marker='s', label="XDP 512")
plt.plot(redir[:,0], redir[:,1]/10**6, marker='s', label="XDP 1024")
...
:results:
# Out[167]:
[[file:./obipy-resources/CzETtD.svg]]
:end:

** XDP_TX

#+NAME: tx-test
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var redir_data=xdp_redirect_data :var tx_data=xdp_tx_data01
tx = np.array(tx_data)
redir = np.array(redir_data)
plt.plot(tx[:,0], tx[:,2]/10**6, marker='o', label="TX")
...
:results:
# Out[170]:
[[file:./obipy-resources/G5CL1B.svg]]
:end:
