...
#+NAME: redirect-test
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var dpdk_data=dpdk_test2 :var xdp_data=xdp_redirect_data :var linux_data=linux_fwd
dpdk = np.array(dpdk_data)
xdp = np.array(xdp_data)
linux = np.array(linux_data)
plt.plot(dpdk[:,0], dpdk[:,2]/10**6, marker='o', label="DPDK")
plt.plot(xdp[:,0], xdp[:,3]/10**6, marker='s', label="XDP")
...
:results:
# Out[8]:
[[file:./obipy-resources/vFMnRC.svg]]
:end:
