...
#+NAME: tx-test
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var redir_data=xdp_base_data :var tx_data=xdp_tx_data01
tx = np.array(tx_data)
redir = np.array(redir_data)
plt.plot(tx[:,0], tx[:,2]/10**6, marker='o', label="TX")
plt.plot(redir[:,0], redir[:,2]/10**6, marker='s', label="REDIRECT")
plt.xlabel("Number of cores")
plt.ylabel("Mpps")
plt.legend()
plt.ylim(0,70)
...
:results:
# Out[160]:
[[file:./obipy-resources/SLyVfn.svg]]
:end:
