...
#+NAME: xdp_drop_data_rx_cqe_compress
| RXQs DROP | no_touch RX=1024 | no_touch RX=512 | read RX=1024 | read RX=512 |
|-----------+------------------+-----------------+--------------+-------------|
...
Observations: Even with these extremely high numbers we are still
seeing idle CPU cycles.
...
Issues: With the rx_cqe_compress=on setting, I'm seeing errors in the
kernel dmesg, and individial RX-queues are stopping to work. (Stopping
and starting XDP application enables queues again).
...
#+NAME: drop-test
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var dpdk_data=dpdk_test2 :var xdp_data=xdp_drop_data_rx_cqe_compress :var linux_data=linux_iptables_drop
...
:results:
# Out[67]:
[[file:./obipy-resources/I3OD3d.svg]]
:end:
...
#+NAME: redirect-test
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var dpdk_data=dpdk_test2 :var xdp_data=xdp_redirect_data :var linux_data=linux_fwd :var tx_data=xdp_tx_data03_256
dpdk = np.array(dpdk_data)
...
:results:
# Out[17]:
[[file:./obipy-resources/NMQMJP.svg]]
:end:

** XDP_TX

#+NAME: tx-test
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var redir_data=xdp_redirect_data :var tx_data=xdp_tx_data03_256
tx = np.array(tx_data)
...
:results:
# Out[9]:
[[file:./obipy-resources/BQycom.svg]]
:end:
