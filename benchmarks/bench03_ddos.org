...
#+BEGIN_SRC ipython :session :exports both :results raw drawer :var data=xdp_ddos_run2 :var no_prot=no_protection
d = np.array(data)
nprot = np.array(no_prot)
plt.plot(d[:,0], d[:,5]/1000, marker='o', label="XDP")
plt.plot(nprot[:,0], nprot[:,1]/1000, marker='s', label="No XDP")
...
:results:
# Out[193]:
[[file:./obipy-resources/i46ttM.svg]]
:end:
