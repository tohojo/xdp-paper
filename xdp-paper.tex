...
ensures the safety of the program by statically verifying it before it is
loaded; and programs are dynamically compiled into native machine instructions
...
\item Is completely transparent to applications running on the host, enabling
  new deployment scenarios such as inline protection against denial of service
...
overhead in one way or another. The approaches taken to this fall into three
broad categories: (a) implementing parts of the application as a module inside
the kernel itself, (b) providing an interface for userspace to access packet
data with lower overhead than with traditional sockets, or (c) bypassing the
kernel entirely and handing over control of the networking device directly to
userspace.
...
addition, such systems generally integrate into the kernel networking stack,
...
We describe each of these components in the following.
...
arrives. The program is executed directly in the device driver, without context
switching to userspace. As shown in Figure~\ref{fig:xdp-kernel}, the program is
executed at the earliest possible moment after a packet is received from the
hardware, before the kernel allocates its per-packet \emph{sk\_buff} data
...
any data structure lookups needed by more generic code.
...
the special \texttt{PACKET\_END} pointer in the context structure. This approach
means that the static analysis can verify that the program never reads outside
the variable-length packet data, without knowing the length in advance. This
enables program instructions to directly access packet data, which is an
...
machine runs a pre-release of the upcoming version 4.18 of the Linux kernel. The
full details of our setup are available, along with links to source code and the
raw test data, in an online repository~\cite{test-data}, to help others
...
As we have seen in the previous sections, XDP achieves significantly higher
...
cards, show a smaller performance delta between XDP and DPDK. However, this
hardware has performance limits that prevent us from showing the maximum
performance of the system, which is why we have not used it for our experiments.
...
cases have all seen deployment in one form or another, although we use
simplified versions in our evaluation so as to be able to make the code
available.
...
feature to pass all other packets to a different CPU core for processing. To
...
opens a TCP connection and sends a small payload that is echoed back from the
...
by announcing a virtual IP address, which is routed to the load balancer. At the
load balancer, the XDP program hashes the source packet header using a stable
hashing algorithm, to select a destination application server. The packet is
then encapsulated and sent to the application server, which is responsible for
decapsulating it, processing the request, and replying directly to the
originator. The XDP program performs the encapsulation, and returns the packet
out the same interface on which it was received.
...
were previously rejected. Furthermore, the error messages reported by the
verifier have been made friendlier, to make it easier for developers to change
their code to avoid verification errors. Support for function calls in eBPF has
recently been added, and the addition of support for bounded loops is planned.
Finally, efficiency improvements that will allow the verifier to operate on
larger programs are also being worked on.
...
outlined a number of developments which will continue to improve it in the
future.
