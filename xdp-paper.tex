...
\begin{abstract}
  ...
  high-performance packet processing applications, executed in the context of the
  kernel device driver. This system, called the eXpress Data Path (XDP), is part
  of the mainline Linux kernel, and has been gradually expanded over the last
  several releases. Such applications are programmed from higher level languages
  such as C, compiled into custom bytecode which the Linux kernel then statically
  verifies for safety and translates into native instructions of the underlying
  architecture. We show that XDP achieves a maximum packet processing performance
  of 25 million packets per second on a single core, and illustrate the flexibility
  of the programming model through three example use cases: inline DDoS protection,
