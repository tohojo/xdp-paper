...
\begin{abstract}
  ...
  mainline Linux kernel, and got gradually expanded over the last releases. In
  XDP, applications are programmed from higher level languages such as C, compiled
  into custom bytecode which the Linux kernel then statically verifies for
  safety, and translates into native instructions of the underlying architecture
  or even transparently offloads into SmartNICs. We show that XDP achieves a
  maximum packet processing performance of 25 million packets per second on
  a single core, and illustrate the flexibility of the programming model
  through three example use cases: inline DDoS protection, layer-3 routing and
