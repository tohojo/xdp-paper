...
\acmPrice{15.00}
\acmDOI{xxxxxxxxxxxxxxxxxxxxxxx}
\acmISBN{xxxxxxxxxxxxxxxxxxxxxx}
%\widowpenalty=100
%\clubpenalty=100
\brokenpenalty=100
...
\begin{abstract}
  ...
  userspace. However, as the operating system is bypassed, so are its familiar
  tooling, application isolation mechanisms and security guarantees; and packet
  processing applications are separated from the rest of the system.
...
system complexity and blurs strict security boundaries otherwise enforced by the
...
execution environment in the form of a virtual machine running a custom byte
code that is based on an extended version of BPF, the original BSD Packet Filter
\cite{mccanne_bsd_1993}. This environment executes custom programs directly in
kernel context, before the kernel itself touches the packet data, which enables
custom processing (including redirection) at the earliest possible point after a
packet is received from the hardware. The kernel ensures the safety of the
custom programs by statically verifying them at load time; and programs are
dynamically compiled into native machine instructions to ensure high
performance.
...
system. This deep integration with the kernel obviously imposes some design
...
different XDP program, thus splitting processing into logical sub-units (based
on, say, IP header version).
...
with it to other XDP programs, as well as to other parts of the kernel and
userspace.
...
codes (with no parameters) to drop the packet, immediately re-transmit it out
the same network interface, or allow it to be processed by the kernel networking
stack. The fourth return code allows the XDP program to \emph{redirect} the
...
understands 22 different instructions. eBPF extends the number of registers from
two to eleven, and increases register widths to 64 bits. The 64-bit registers
map one-to-one to hardware registers on the 64-bit architectures supported by
the kernel, which allows efficient just-in-time (JIT) compilation into native
machine code. Support for compiling (restricted) C code into eBPF is included in
the LLVM compiler infrastructure~\cite{llvm}.
...
While the eBPF instruction set itself supports general purpose computation, the
verifier (described in Section~\ref{sec:bpf-verifier} below) places limitations
on the programs loaded into the kernel, to ensure that the user-supplied
programs they cannot harm the running kernel. With this in place, it is safe to
execute the code directly in the kernel address space, which makes eBPF useful
for a wide variety of tasks in the Linux kernel, of which XDP is one. Because
all eBPF programs can share the same set of maps, this makes it possible for XDP
programs to react to arbitrary events in other parts of the kernel. For
instance, a separate eBPF program could monitor CPU load and instruct the XDP
...
program context. Instead, the kernel exposes helper functions allowing programs
to access \emph{BPF maps}.
...
approach: It verifies that the program being loaded does its own bounds checking
before dereferencing pointers to packet data, and that map lookups are checked
for \texttt{NULL} values before being dereferenced. With this verification in
...
programs. This approach also leaves the program writer in control of how bounds
checking is integrated into the processing logic, and what to do in the error
path.
...
pointer to the \emph{context} metadata object, \texttt{R10} is a stack pointer,
and all other registers are marked as not initialised. At each execution step,
register states are updated based on the operations performed by the program.
When a new value is stored in a register, that register inherits the state
variables from the source of the value. Arithmetic operations will affect the
possible value ranges of scalar types, and the offset of pointer types. The
widest possible range is stored in the state variables, e.g., if a one-byte load
is performed into a register, that register's possible value range is set to
0-255. Branches in the instruction graph will update the register state
according to the logical operation contained in the branch. For example, given a
comparison such as "\texttt{R1 > 10}", the verifier will set the maximum value
of \texttt{R1} to 10 in one branch, and the minimum value to 11 in the other.
...
arithmetic (depending on the type of pointer), and pointers cannot generally be
converted to integer values (to prevent leaking them to userspace). Any eBPF
program that performs operations that the verifier cannot prove are safe, are
simply rejected at load time. In addition to this, the verifier also uses the
range information to enforce aligned memory accesses.
...
\item The map lookup helper function exposed by the kernel (called on line 60;
  all other functions are inlined on compilation, including helpers like
  \texttt{htons()}).
...
the selected use cases in XDP, not to perform an exhaustive performance
evaluation against state of the art implementations of each use case. Instead,
we use the regular Linux kernel stack as a simple performance baseline and
benchmark the XDP applications against that.
...
which it was received. It keeps configuration data in BPF maps and implements
the encapsulation entirely in the eBPF program.
...
structure imposed by the verifier, such as the lack of support for loops, have
been more limited than strictly necessary, in the interest of ensuring safety.
...
We also extend our thanks to the anonymous reviewers, and to our shepherd
Srinivas Narayana, for their helpful comments on the paper.
