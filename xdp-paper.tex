...
increasing number of cores dedicated to packet processing.\footnote{The
  \emph{Hyperthreading} feature of the CPU is disabled for our experiments, so
  whenever we refer to the number of CPU cores, this means the number of
  physical cores.} For XDP and the Linux network stack (which do not offer an
explicit way to dedicate cores to packet processing) we achieve this by
configuring the hardware Receive Side Scaling (RSS) feature to steer traffic to
the desired number of cores for each test.
