...
flexibility, and to achieve this they perform too many operations per packet to
be able to keep up with these high packet rates. This has led to the increased
popularity of special-purpose toolkits for software packet processing, such as
the Data Plane Development Kit (DPDK)~\cite{dpdk}. Such toolkits generally
bypass the operating system completely, instead passing control of the network
hardware directly to the network application and dedicating one, or several, CPU
cores exclusively to packet processing.
...
results in increased systems complexity, management and maintainability costs,
and at the same time blur strict security boundaries otherwise enforced by the
...
This environment executes custom programs directly in the device driver context,
before the kernel performs any other packet processing tasks. The kernel ensures
safety performance. The former by statically verifying the program before it is
loaded, and the latter by dynamically compiling the virtual machine bytecode
...
presented before. In this work we present such a high-level design description
of XDP and its capabilities, and how it integrates with the rest of the Linux
kernel. Our performance evaluation shows raw packet processing performance of up
to 24 million packets per second per CPU core, which does not quite match the
...
programmability, the flexibility and low cost of common off-the-shelf (COTS)
hardware has obvious advantages, and as performance continues to increase while
cost decreases, packet processing systems running on COTS hardware become ever
more common. Examples of this include applications performing single functions,
such as switching~\cite{rizzo2012vale}, routing~\cite{han2010packetshader},
named-based forwarding~\cite{kirchner2016augustus},
classification~\cite{santiago2012wire}, caching~\cite{mansilha2015hierarchical}
or traffic generation~\cite{emmerich2015moongen}. They also include more general
solutions which are highly customisable and can operate on packets from a
variety of
...
thereof. In contrast to this, XDP is a lower-level framework that focuses on how
to build a high-performing base for packet processing applications to run on. To
...
system, often partly or completely disabling the regular network stack. In
addition, even if the frameworks mentioned above improve performance
...
(DPDK)~\cite{dpdk}. This latter started out as an Intel-specific hardware
support package, but has since seen a wide uptake under the stewardship of the
Linux Foundation and represents the state of the art in high-performance packet
processing. The kernel bypass approach offers excellent performance, but has the
drawback that it is more difficult to integrate with the rest of the system, as
...
verified to not adversely affect the functioning of the rest of the kernel, most
of the fragility issues normally faced by in-kernel packet processing
implementations are avoided. In addition, the byte code format and execution
context are part of the kernel Application Binary Interface (ABI) to userspace,
which means that the usual ABI stability guarantees apply; and so applications
do not have to keep up with future changes in the kernel to be able to continue
to function.
...
running in userspace. This include the ability to bypass parts of the network
...
\caption{\label{fig:xdp-execution} Execution flow of a typical XDP program. When
  ...
  the packet determined. The program can alternate between packet parsing,
...
adding custom packet processing as required. This section describes how the
components of XDP work, and how they fit together to create the full system.
This is illustrated by Figure~\ref{fig:xdp-kernel}, which shows a diagram of how
XDP integrates into the Linux kernel, and Figure~\ref{fig:xdp-execution}, which
shows the execution flow of a typical XDP program.

There are four major components in the XDP system:
...
arrives. The program is executed directly in context of the device driver,
without context switching to userspace. As shown in Figure~\ref{fig:xdp-kernel},
the program is executed at the earliest possible moment after a packet is
received from the hardware, before the kernel allocates its per-packet
\emph{sk\_buff} data structure or performs any parsing of the packet. This
allows for high performance, but the program has do more work, and parse raw
packet data itself.
...
data, along with metadata fields describing which interface and receive queue
the packet came in on, etc.
...
pointer to a special memory area, that XDP programs can use to store arbitrary
...
the right-hand side of Figure~\ref{fig:xdp-execution}. There are three simple
return codes which either drops the packet, immediately re-transmits it out the
same network interface, or allows the packet to be processed by the kernel
networking stack. In addition to these simple actions, the XDP program can
\emph{redirect} the packet, which offers additional control over its further
processing.
...
actions. The original virtual machine has two 32-bit registers and understands
22 different instructions. This makes BPF well-suited for packet filtering
...
instance, and XDP program could drop packets if processing load increases above
a certain threshold.
...
other programs and never returns to the caller.
...
possible to limit the amount of processing performed for a given situation, by
completely removing parts of the program that are not needed, and re-loading it
if as requirements change. The dynamic loading of programs also makes it
straight-forward to express processing rules directly in program code, avoiding
potentially expensive data structure lookups that are needed in more generic
code.
...
happening, the kernel enforces a single entry point for loading all BPF programs
(through the \texttt{bpf()} system call). When loading a BPF program it is first
analysed by the in-kernel eBPF verifier. The verifier performs a static analysis
of the program byte code to ensure that the program performs no actions that are
unsafe (such as accessing arbitrary memory), and that the program will
terminate. The latter is ensured by disallowing loops and limiting the maximum
program size. The verifier works by first building a directed acyclic graph
(DAG) of the control flow of the program. This DAG is then verified as follows:
...
the program. When a new value is stored to a register, it inherits the state
variables of the source of the value. Arithmetic operations on scalar values
will affect the value of the \emph{tnum} state variable, which tracks which bits
in a register are known, and their value. The \emph{tnum} is a pair of bit
masks: A \emph{mask}, which contains the bits whose value is unknown, and a
\emph{value} which contains the bits that are known to be set to 1. Load
operations set these, for instance loading a single byte from memory will result
in the top 56 bits being known to be zero, and the bottom 8 bits to be unknown.
Arithmetic operations update these values according to their effect on the value
...
to compute the possible ranges of values in each of the branches (using the
minimum and maximum possible values of unknown bits as appropriate). Finally, a
branch that checks whether a register with a \texttt{MAP\_VALUE\_OR\_NULL}
pointer is different from \texttt{NULL} will turn that register into a pointer
with type \texttt{MAP\_VALUE} in the \emph{true} branch, which makes it possible
...
performance of eBPF programs that are used for packet processing.
...
essential areas, while integrating with the kernel and making full use of its
...
benchmarks to examine specific aspects of system's capabilities. For all
...
machine runs a pre-release of the upcoming version 4.18 of the Linux kernel.
...
configuration issues are omitted here due to space constraints, but we make them
...
over the fastest processing mode of the regular networking stack, achieving
24\,Mpps on a single core.
...
perform it, the results are not directly comparable. For this reason, we omit
the Linux networking stack from these results, and instead include the Linux
routing performance in our real-world use case presented in
...
when packets are sent out on the same interface that it was received on,
especially as the number of cores in use increases. This is primarily due to
differences in memory handling: packet buffers are allocated by the device
driver and associated with the receiving interface. And so, when the packet is
forwarded out a different interface, the memory buffer needs to be returned to
the interface that it is associated with. This has some overhead, although work
is ongoing to reduce it.
...
driver. Indeed other drivers, such as the \emph{i40e} driver for 40\,Gbps Intel
cards, show a smaller performance delta between XDP and DPDK.
...
kernel routing table with all next-hop addresses set to the address of the test
system connected to our egress interface. The routing table dump contains
752,138 distinct routes, and for our tests we generate 4000 random destination
IP addresses to make sure the full lookup table is exercised.
...
rate on a 10\,Gbps link using a single core (using a conservative estimate of an
average packet size of 300\,bytes).
...
attacks directly at the application servers, without needing to change
applications. In the case of a virtual machine deployment, the filter can even
be installed in the hypervisor, and thus protect all virtual machines running on
the host.
...
benchmarking tool~\cite{netperf}. Netperf contains a TCP-based round-trip
...
offer an increasing load of small UDP packets matching our packet filter, which
simulates the DoS attack. We measure the number of TCP transactions achieved as
the attack traffic volume increases, repeating each experiment four times and
...
rapidly. This shows that effective DDoS filtering is quite feasible to perform
in XDP, which comfortably handles 10\,Gbps of minimum-packet DoS traffic on a
single CPU core. Deploying DDoS mitigation this way leads to increased
flexibility, since neither application changes or additional hardware is needed.
...
machine. This performance is shown in Table~\ref{tbl:load-balancer}, which, as
in the other examples, shows linear scaling with the number of CPUs, and the
ability to saturate a 10\,Gbps link using three CPU cores.
...
familiar with the kernel ecosystem and ``just wants to write packet processing
programs'' can find this ecosystem of kernel-specific tools a limitation, as
others have noted~\cite{miano2018creating}. To ease the transition, a wide
variety of tools exist, including the BPF Compiler Collection~\cite{bcc}, the
\emph{bpftool} introspection program~\cite{bpftool} and the \emph{libbpf}
library of utility functions~\cite{libbpf}. These have already seen significant
improvements since their inception, but more work is needed in this area.
...
from programs being loaded into the kernel. As such, a conservative approach is
taken, where the verifier will reject any program that it cannot prove is safe.
This can lead to false negatives, where safe programs are needlessly rejected.
...
To improve upon this situation, there is an ongoing effort to teach the verifier
to recognise more programs as safe that were previously rejected as invalid. In
addition, the error messages reported by the verifier have been made friendlier,
to make it easier for developers to change their code to avoid verification
errors. In addition, support for function calls in eBPF has recently been added,
and the addition of support for bounded loops is planned. Efficiency
improvements that will allow the verifier to operate on much larger programs are
...
on the hardware, which means that full support in all drivers is possible; and
indeed we believe that is how this limitation will eventually be lifted.
...
the Generic XDP feature~\cite{generic-xdp} makes it possible to run XDP programs
even if the networking driver lacks the proper support, by moving execution into
the core networking stack (which naturally reduces performance).
...
which controls mapping of memory regions for device DMA access. In the current
...
regions, while other parts can be amortised through batching.
...
capacity. This limitation can be exacerbated when joining networks with
different speeds or other mismatched network characteristics.
...
processing directly into the operating system kernel.

Our evaluation has shown that XDP achieves raw packet processing performance of
up to 24 million packets per second on a single CPU core. While this is not
quite on par with state of the art kernel bypass-based solutions, we argue that
there is nothing preventing XDP from fully achieving the same performance; and
...
all-or-nothing kernel bypass solutions, which is supported by its adoption in a
variety of real-world applications, some of which we have shown examples of
above. The XDP system is continually evolving, and we have outlined a number of
developments which will make XDP an even more compelling platform in the future.
