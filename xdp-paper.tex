...
machine runs a pre-release of the upcoming version 4.18 of the Linux kernel. To
help others reproduce our results, we make available the full details of our
setup, along with links to source code and the raw test data, in an online
...
driver. Indeed with other drivers, such as the \emph{i40e} driver for 40\,Gbps
Intel cards, we have seen a smaller performance delta between XDP and DPDK.
However, this hardware has performance limits that prevent us from showing the
maximum performance of the system, which is why we have not used it for the
