...
implementing the selected use cases in XDP. For this reason we consider an
exhaustive performance evaluation against other implementations of the same use
cases to be out of scope, and only benchmark the XDP applications against the
...
As others similar approaches have shown~\cite{stackmap}, there is significant
potential in improving the packet processing performance of around the TCP stack
itself. There has been some initial discussion of how this could be achieved for
XDP~\cite{txdp}; while far from trivial, this presents an exciting opportunity
for expanding the scope of the XDP system.
