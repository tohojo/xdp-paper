...
\begin{abstract}
  ...
  solution working in concert with the kernel's networking stack. In XDP,
...
\item Integrates cooperatively with the regular networking stack, retaining full
  ...
  tools are needed. In addition, any network adapter with a Linux driver can
  be supported by XDP; no special hardware features or from scratch driver
...
\item Does not require expensive packet reinjection from user space into
  kernel space when workloads based on the normal socket layer need to be
  reached, since XDP already operates in kernel space.
