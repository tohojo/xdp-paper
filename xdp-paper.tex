...
\begin{listing}
  \caption{\label{lst:ex-xdp-prog}Example XDP program. From xdp2\_kern.c,
    distributed with the kernel source code. Some lines are compressed slightly to fit on
    the page, and the parse\_ipv6() function is omitted.}
...
\begin{itemize}
\item A BPF map is defined (lines 1--6) for keeping statistics of the number of
  processed packets. The map is keyed on IP protocol number and each value is
  simply a packet count (updated in lines 65--67). A userspace program can poll
...
\item Pointers to the start and end of the packet data is read from the context
  object (lines 28--29), to be used for direct packet data access.
\item Checking against the data\_end pointer ensures that no data is read out of
  bounds (lines 20, 34, 44 and 53).
\item Direct packet data access is used to modify the packet headers (lines
  12--14).
\item Functions defined inside the program (lines 8--23) which are inlined by
  the compiler (automatically in this case).
\item The use of helper functions exposed by the kernel (in this example just
  the map lookup function called on line 65).
\item Packet verdict communicated by the program return code (line 73).
\end{itemize}
...
bounds checking and (c) that the return value from the map lookup is checked
against NULL before it is accessed.
...
Note that the purpose of this section is to demonstrate the feasibility of
implementing the selected use cases in XDP. For this reason we do not perform an
exhaustive performance evaluation against other implementations of the same use
cases, but only benchmark the XDP applications against the baseline Linux
kernel. An exhaustive performance evaluation is out of scope for this paper.
...
explained in Section~\ref{sec:reg-state}.
