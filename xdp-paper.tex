...
systems are typically optimised for flexibility, and to achieve this they
perform too many operations per packet to be able to keep up with these high
packet rates. This has led to the increased popularity of special-purpose
toolkits for software packet processing, such as the Data Plane Development Kit
...
before the kernel performs any other packet processing tasks. The kernel also
ensures the safety and performance of the program. The former by statically
verifying the program before it is loaded, and the latter by dynamically
compiling the virtual machine bytecode into native machine instructions.
...
significantly, they still lag behind in performance compared to kernel bypass
...
\emph{sk\_buff} data structure or performs any parsing of the packet.
...
of the kernel and userspace. Additionally, an XDP program also has access to
kernel facilities that provide helper functions and additional metadata. These
allow the XDP program to gather additional metadata through helper functions and
maps (dotted lines in Figure~\ref{fig:xdp-kernel} and the top part of
Figure~\ref{fig:xdp-execution}). New helpers are actively been added by the
...
registers map one-to-one to hardware registers on the 64-bit architectures
...
as requirements change. The dynamic loading of programs also makes it
...
masks: a \emph{mask} that contains the bits whose value is unknown, and a
...
branch, which makes it possible to derefence the pointer.
...
For pointers to context objects, the execution context of an eBPF program
...
packet length are allowed. The latter is enforced by ensuring that any reading
or writing of packet data is preceded by an appropriate range check, through
comparison with the special \texttt{PACKET\_END} pointer in the context
structure. This approach means that the static analysis can verify that the
program never reads outside the variable-length packet data, without knowing the
length in advance. This enables program instructions to directly access packet
data, which is an important reason for the high performance of eBPF packet
processing programs.
...
\item Packet forwarding performance. A packet processing system that cannot
  forward packets has limited utility. However, since forwarding introduces an
...
increasing number of cores dedicated to packet processing. For XDP and the Linux
network stack (which do not offer an explicit way to dedicate cores to packet
processing) we achieve this by configuring the hardware Receive Side Scaling
(RSS) feature to steer traffic to the desired number of cores for each test.
...
compared to the other systems in Section~\ref{sec:perf-discussion}.
...
packets; this lookup is expensive, and since the other applications do not
...
the same interface as they arrived on can be used to implement a layer-3 load
balancer.
...
which allows performing full routing table lookups directly from XDP. The result
of the lookup is an egress interface and a next-hop MAC address, which makes it
...
which will perform the neighbour lookup, allowing subsequent packets to be
forwarded by XDP.
...
feature to pass all other packets on for processing on a different CPU core. To
simulate a baseline application load we use the Netperf benchmarking
tool~\cite{netperf}. Netperf contains a TCP-based round-trip benchmark, which
opens a TCP connection and sends a small payload which is echoed back from the
server, repeating as soon as a reply is received. The output is the number of
transactions per second, which represents performance of an interactive use
case, such as small remote procedure calls.
...
simulates the DoS attack. We measure the TCP transactions performance as the
...
of destination hosts\footnote{We use one virtual IP per CPU core, and 100
  destinations per virtual IP.}, and run it on our test machine. This
performance is shown in Table~\ref{tbl:load-balancer}, which, as in the other
examples, shows linear scaling with the number of CPUs, and the ability to
...
an overview of some of these efforts. These include: User experience and
debugging; driver support; performance improvements; QoS and better handling of
rate transitions; and acceleration of transport protocols. The following
subsections discuss each effort in turn.
...
from programs being loaded into it. As such, a conservative approach is
...
In addition, as others have noted~\cite{miano2018creating}, the restrictions on
the program structure imposed by the verifier, such as the lack of support for
...
ongoing effort to teach the verifier to recognise more programs as safe that
were previously rejected as invalid. Furthermore, the error messages reported by
the verifier have been made friendlier, to make it easier for developers to
...
the Generic XDP feature~\cite{generic-xdp} makes it possible to run (reduced
performance) XDP programs even if the networking driver lacks the proper
...
which controls mapping of memory regions for DMA access. In the current design,
there is some unnecessary overhead associated with this. Improved tracking of
packet memory can eliminate some of this overhead entirely, while the rest can
be amortised through batching.
...
above. The XDP system is still evolving, and we have outlined a number of
developments which will continue to improve it in the future.
