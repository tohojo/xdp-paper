...
load balancer. At the load balancer, the XDP program hashes the source packet
header using a stable hashing algorithm, to select a destination application
server. The packet is then IPIP-encapsulated and sent to the application server,
which is responsible for decapsulating it, processing the request, and replying
directly to the originator of the request. This means that the XDP program can
perform the packet encapsulation, and immediately return the packet out the same
interface.
